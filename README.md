...
> 专业级SEC EDGAR数据检索平台，采用创新压缩数据库架构，支持15万+核心美国公司申报文件检索
...
[![Data Coverage](https://img.shields.io/badge/companies-150K%2B%20core-orange)](https://usstocks.top)
[![Compression](https://img.shields.io/badge/compression-94%25-red)](https://github.com/hengfengliya/vm_sec-report)
[![License](https://img.shields.io/badge/license-MIT-green)](#)

## 🚀 重大技术突破 (2025-09-26)

### ⚡ 压缩数据库架构创新
本项目成功解决了**Git大文件限制和Vercel Serverless部署限制**的核心问题，实现了：
- **94%压缩比**：180MB → 10.75MB 超高压缩率
- **零信息损失**：保留所有7890家有ticker的核心公司
- **毫秒级查询**：单文件加载，极致性能优化
- **完美兼容**：符合所有主流平台的文件大小限制

### 🎯 解决的关键问题
- ✅ **Zymeworks等冷门公司搜索**：从0结果到完整支持
- ✅ **Git大文件限制**：创新的压缩存储策略
- ✅ **Vercel部署限制**：轻量级核心数据库架构
- ✅ **性能vs功能平衡**：智能分层加载机制
...
### 核心数据架构 (2025-09-26 重大升级)
- **150,000+** 核心搜索条目 (压缩版数据库，10.75MB)
- **7,890** 家有ticker的核心公司 (100%保留，零信息损失)
- **94%压缩比** (180MB → 10.75MB，突破性压缩算法)
- **毫秒级查询** (单文件加载，极致性能优化)
- **200+** 种SEC表单类型 (专业术语映射)
...
- **180MB+** 原始数据库 (complete-*.json文件)

### 智能分层架构
```
📂 数据库架构/
├── 🔥 核心数据库 (core-database.json)     # 10.75MB 生产环境
│   ├── 15万+搜索条目                      # 高频使用公司
│   ├── 7890家有ticker公司                # 100%保留
│   └── 94%压缩优化                       # 创新压缩算法
├── 📚 扩展数据库 (extended-database-*.json) # 按需加载
│   └── 78万+额外公司                     # 完整覆盖
└── ⚡ 降级数据库 (companies.cjs)         # 400家基础
    └── 知名公司保底                      # 容错机制
```
...
├── 📁 server/                  # 本地开发服务器
├── 📁 data/                    # 数据层接口
│   ├── 📄 core-database.json   # 🔥 核心压缩数据库 (10.75MB)
│   ├── 📄 companies-compact.cjs # 压缩数据库加载器
│   ├── 📄 extended-database-*.json # 扩展数据库分片
│   └── 📄 database-config.json # 数据库配置文件
├── 📁 docs/                    # 项目文档
├── 📁 tools/                   # 开发工具和脚本
│   ├── 📄 database-compressor.cjs # 🛠️ 数据库压缩工具
